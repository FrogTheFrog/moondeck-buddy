...
namespace
{
const QString REG_STEAM_PATH{"HKEY_CURRENT_USER/Software/Valve/Steam"};
const QString REG_STEAM_APPS_PATH{"HKEY_CURRENT_USER/Software/Valve/Steam/Apps"};
...
void PcControlImpl::launchSteamApp(uint app_id)
{
    ...
    if (!steam_is_running || m_current_app_id != app_id)
    {
                    ...
                    if (changed_values.contains(REG_APP_RUNNING))
                    {
                        is_running =
                            (changed_values[REG_APP_RUNNING].isValid() ? changed_values[REG_APP_RUNNING].toBool()
                                                                       : false);
                        qDebug("App %s \"running\" value change detected: %d", qUtf8Printable(QString::number(app_id)),
                               static_cast<int>(is_running));
                    }
                    if (changed_values.contains(REG_APP_UPDATING))
                    {
                        is_updating =
                            (changed_values[REG_APP_UPDATING].isValid() ? changed_values[REG_APP_UPDATING].toBool()
                                                                        : false);
                        qDebug("App %s \"updating\" value change detected: %d", qUtf8Printable(QString::number(app_id)),
                               static_cast<int>(is_updating));
                    }

                    if ((is_running || is_updating) && m_current_app_id != app_id)
                    {
                        m_current_app_id = app_id;
                        qDebug("Running appID change detected (via app key): %s",
                               qUtf8Printable(QString::number(getRunningApp())));
                    }
                    else if (m_current_app_id != FALLBACK_APP_ID)
                    {
                        m_current_app_id = FALLBACK_APP_ID;
                        qDebug("Running appID change detected (via app key): %s",
                               qUtf8Printable(QString::number(getRunningApp())));
                    }
                });

        m_app_reg_key->open(REG_STEAM_APPS_PATH + "/" + QString::number(app_id),
...
{
    return m_current_app_id.value_or(FALLBACK_APP_ID);
}
...
void PcControlImpl::slotHandleRegKeyChanges(const QMap<QString, QVariant>& changed_values)
{
    ...
    {
        const auto prev_app_id = m_current_app_id;
        m_current_app_id       = changed_values[REG_APP_ID].isValid()
                                     ? std::make_optional(changed_values[REG_APP_ID].toUInt())
                                     : std::nullopt;

        if (prev_app_id != m_current_app_id)
        {
            qDebug("Running appID change detected (via global key): %s",
                   qUtf8Printable(QString::number(getRunningApp())));
        }

        if (m_app_reg_key != nullptr)
        {
            m_app_reg_key = nullptr;
            qDebug("Clearing app reg change watcher.");
        }
    }
}
