...
const QMap<steam::SteamId::Type, std::vector<QString>>& getTypeLetterMap()
{
    using enum steam::SteamId::Type;
    static const QMap<steam::SteamId::Type, std::vector<QString>> type_letters{
...
QString getTypeLetter(const steam::SteamId::Type type)
{
    return getTypeLetterMap().value(type, {"I"}).at(0);
}

std::optional<steam::SteamId::Type> getTypeFromLetter(const QString& letter_to_find)
{
...
std::optional<SteamId> SteamId::fromString(const QString& str)
{
    ...
    if (const auto match{steam_3_id_regex.match(str)}; match.hasMatch())
    {
        ...
        if (!converted)
        {
            qCWarning(lc::steam) << "Could parse Steam32 ID from" << str;
            return std::nullopt;
        }
                                             ...
                                             std::uint32_t account_number, std::uint32_t parity_bit)
{
    if (!universe)
    {
        qCWarning(lc::steam) << "Could parse SteamID Universe from" << original_str;
        return std::nullopt;
    }

    if (!type)
    {
        qCWarning(lc::steam) << "Could parse SteamID Type from" << original_str;
        return std::nullopt;
    }
