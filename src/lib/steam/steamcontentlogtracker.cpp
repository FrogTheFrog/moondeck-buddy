// header file include
#include "steam/steamcontentlogtracker.h"
...
namespace
{
std::map<AppId, SteamContentLogTracker::AppState>
    remapStateChanges(const std::map<AppId, QVector<SteamContentLogTracker::AppStateChange>>& input)
{
    ...
    std::map<AppId, AppState> new_app_states;
    for (const auto& [app_id, state_changes] : input)
    {
        qCDebug(lc::steam) << "New state changes for AppID" << app_id.getId() << "detected:" << state_changes;
...
void SteamContentLogTracker::onLogChanged(const std::vector<QString>& new_lines)
{
    ...
    for (const QString& line : new_lines)
    {
        ...
        if (match.hasMatch())
        {
            ...
            for (const auto& state : change_states)
            {
                ...
                {
                    qCWarning(lc::steam) << "Unmapped state for AppID" << app_id->getId() << "found:" << state;
                    continue;
                }
    ...
    for (const auto& [app_id, app_state] : new_app_states)
    {
        ...
        qCInfo(lc::steam) << "New app state for AppID" << app_id.getId() << "detected:" << enums::qEnumToString(it->second)
                       << "->" << enums::qEnumToString(app_state);
        if (app_state == AppState::Stopped)
        {
