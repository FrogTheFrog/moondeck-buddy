...
std::optional<shared::AppId::IdType> guessGameId(const std::uint64_t app_or_game_id)
{
    switch (getTypeValue(app_or_game_id))
    {
        case game_mod_game_id_type:
            ...
            {
                return shared::AppId::IdType::ModGameId;
            }
            break;
        case shortcut_game_id_type:
            if (getModId(app_or_game_id) != 0U)
            {
                return shared::AppId::IdType::ShortcutGameId;
            }
            break;
        default:
...
shared::AppId::IdType guessAppIdType(const std::uint64_t app_or_game_id)
{
    ...
    return app_or_game_id == getAppId(app_or_game_id) ? shared::AppId::IdType::SteamApp
                                                      : shared::AppId::IdType::NonSteamApp;
}
}  // namespace
