    ...
    : m_current_app{app}
{
    ...
    QTimer::singleShot(0, this,
                       [this]()
                       {
                           qCDebug(lc::common) << "getAppName() >>" << getAppName();
                           qCDebug(lc::common) << "getLogDir() >>" << getLogDir();
                           qCDebug(lc::common) << "getLogName() >>" << getLogName();
                           qCDebug(lc::common) << "getLogPath() >>" << getLogPath();
                           qCDebug(lc::common) << "getSettingsDir() >>" << getSettingsDir();
                           qCDebug(lc::common) << "getSettingsName() >>" << getSettingsName();
                           qCDebug(lc::common) << "getSettingsPath() >>" << getSettingsPath();

                           for (const auto& value : enums::qEnumValues<AutoStartDelegation>())
                           {
                               qCDebug(lc::common).nospace() << "getAutoStartDir(" << enums::qEnumToString(value)
                                                             << ") >> " << getAutoStartDir(value);
                               qCDebug(lc::common).nospace() << "getAutoStartName(" << enums::qEnumToString(value)
                                                             << ") >> " << getAutoStartName(value);
                               qCDebug(lc::common).nospace() << "getAutoStartPath(" << enums::qEnumToString(value)
                                                             << ") >> " << getAutoStartPath(value);
                           }

                           qCDebug(lc::common) << "getAutoStartExec() >>" << getAutoStartExec();
                           qCDebug(lc::common) << "getSharedEnvRegexKey() >>" << getSharedEnvRegexKey();
                           qCDebug(lc::common) << "getSharedEnvMapKey() >>" << getSharedEnvMapKey();
                           qCDebug(lc::common) << "isGuiEnabled() >>" << isGuiEnabled();
                       });
}
