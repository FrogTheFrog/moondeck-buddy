                  ...
                  QProcess& process)
{
    ...
    qCInfo(lc::os).nospace() << (env_overrides.empty() ? "" : "[WITH ENV OVERRIDES] ") << "Executing: " << exec
                             << " with args: " << args;
    return true;
}
                               ...
                               const QMap<QString, QString>& env_overrides = {})
{
    ...
    {
        qCWarning(lc::os) << "waitForStarted() failed with" << process.errorString();
        return std::nullopt;
    }
    ...
    {
        qCWarning(lc::os) << "waitForFinished() timed out - killing executable!";
        process.close();
        return std::nullopt;
    }
    ...
    {
        qCInfo(lc::os).noquote() << "Captured stderr:\n-----\n" << QString{output} << "\n-----";
    }

    if (const auto exit_code{process.exitCode()}; exit_code != 0)
    {
        qCWarning(lc::os) << "Executable exited with error code" << exit_code;
        return std::nullopt;
    }

    if (auto output{process.readAllStandardOutput().trimmed()}; !output.isEmpty())
    {
        qCDebug(lc::os).noquote() << "Captured stdout:\n-----\n" << QString{output} << "\n-----";
        return output;
    }
