...
std::optional<std::set<QString>> SunshineApps::load()
{
    ...
    {
        qCWarning(lc::buddyMain) << "file" << filepath << "could not be opened! Reason:" << file.errorString();
        return std::nullopt;
    }
    ...
    qCDebug(lc::buddyMain).noquote() << "Sunshine apps file content:" << Qt::endl
                                     << json_data.toJson(QJsonDocument::Indented);
    if (json_data.isObject())
    {
        ...
        if (apps_it != json_object.end() && apps_it->isArray())
        {
            ...
            if (apps.isEmpty())
            {
                qCDebug(lc::buddyMain) << "there are no Sunshine apps to parse.";
                return parsed_apps;
            }
            ...
            for (const auto& app : apps)
            {
                if (!app.isObject())
                {
                    qCDebug(lc::buddyMain) << "skipping entry as it's not an object:" << app;
                    continue;
                }
                ...
                {
                    qCDebug(lc::buddyMain) << "skipping entry as it does not contain \"name\" field:" << app_object;
                    continue;
                }

                if (!name_it->isString())
                {
                    qCDebug(lc::buddyMain)
                        << "skipping entry as the \"name\" field does not contain a string:" << *name_it;
                    continue;
                }
            ...
            qCDebug(lc::buddyMain) << "parsed the following Sunshine apps:"
                                   << QSet<QString>{std::begin(parsed_apps), std::end(parsed_apps)};
            return parsed_apps;
        }
    }

    qCWarning(lc::buddyMain) << "file" << m_filepath << "could not be parsed!";
    return std::nullopt;
}